...
router.post('/friendRequest/:id/:userId', async (req, res) => {
  try {
    ...
    const friendRequestPending = await FriendStatus.find({ "requestedBy": req.params.id, "userId": req.params.friendRequestId, "friendStatus": 'Pending' });
    ...
    const checkReverseSendRequest = await FriendStatus.find({ "userId": req.params.id, "requestedBy": req.params.friendRequestId, "friendStatus": 'Pending' });
